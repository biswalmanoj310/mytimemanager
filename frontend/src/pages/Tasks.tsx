...
export default function Tasks() {
  ...
  const handleEditTask = async (task: ProjectTaskData) => {
    ...
    // Ensure milestones are loaded for this task's project FIRST
    if (task.project_id) {
      await loadProjectMilestones(task.project_id);
    }
    // Set the task and show modal together using a callback
    setEditingTask(task);
  ...
  return (
    <div className="tasks-page">
          ...
          {!selectedProject ? (
            ...
            <>
              ...
              <div className="project-milestones-section" style={{ marginBottom: '30px' }}>
                ...
                >
                  <h3 style={{ margin: 0 }}>
                    {expandedSections.milestones ? '▼' : '▶'} 🎯 Milestones ({projectMilestones.length})
                  </h3>
                  <button 
                    className="btn btn-secondary" 
                    onClick={(e) => {
                  ...
                  <div className="project-tasks-tree">
                    {!projectTasks || projectTasks.length === 0 ? (
                      ...
                      <div className="task-list">
                        {getTasksByParentId(null).filter(task => task.project_id === selectedProject.id).map((task) => (
                          <TaskNode 
                          key={task.id} 
                          task={task} 
                          level={0}
                          allTasks={projectTasks.filter(t => t.project_id === selectedProject.id)}
                          expandedTasks={expandedTasks}
                ...
                <table className="tasks-table daily-table">
                  <thead style={{ 
                    ...
                    <tr>
                      <th className="col-task sticky-col sticky-col-1" style={{ color: '#ffffff', padding: '12px 8px', fontWeight: 600, fontSize: '13px', textAlign: 'left', background: '#667eea' }}>Task</th>
                      <th className="col-time sticky-col sticky-col-2" style={{ color: '#ffffff', padding: '12px 8px', fontWeight: 600, fontSize: '13px', textAlign: 'center', background: '#4299e1' }}>Ideal<br/>Average/Day</th>
                      <th className="col-time sticky-col sticky-col-3" style={{ color: '#ffffff', padding: '12px 8px', fontWeight: 600, fontSize: '13px', textAlign: 'center', background: '#48bb78' }}>Total Count</th>
                      <th className="col-time sticky-col sticky-col-4" style={{ color: '#ffffff', padding: '12px 8px', fontWeight: 600, fontSize: '13px', textAlign: 'center', background: '#ed8936' }}>Days Done</th>
                      {weekDays.map(day => (
                        <th key={day.index} className="col-hour" style={{ color: '#ffffff', padding: '12px 8px', fontWeight: 600, fontSize: '13px', textAlign: 'center' }}>{day.label}</th>
                      ))}
                      <th className="col-status" style={{ color: '#ffffff', padding: '12px 8px', fontWeight: 600, fontSize: '13px', textAlign: 'center' }}>Actions</th>
                    ...
                    {countBasedTasks.map((task) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="col-time sticky-col sticky-col-2" style={{ textAlign: 'center', ...(bgColor ? { backgroundColor: bgColor } : {}) }}>
                            {task.target_value || 0} {task.unit}
                          </td>
                          
                          <td className="col-time sticky-col sticky-col-3" style={{ textAlign: 'center', ...(bgColor ? { backgroundColor: bgColor } : {}) }}>
                            {totalCount} {task.unit}
                          </td>
                          
                          <td className="col-time sticky-col sticky-col-4" style={{ textAlign: 'center', ...(bgColor ? { backgroundColor: bgColor } : {}) }}>
                            {daysDone}/7 days
                          </td>
              ...
              <form onSubmit={async (e) => {
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    className="form-control"
                    required
                    defaultValue={editingProject?.start_date || formatDateForInput(new Date())}
                  />
                  <small className="form-text">
      ...
      {showEditProjectMilestoneModal && selectedProject && editingMilestone && (
        <div className="modal-overlay" onClick={() => { setShowEditProjectMilestoneModal(false); setEditingMilestone(null); }}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Edit Milestone</h2>
              <button className="btn-close" onClick={() => { setShowEditProjectMilestoneModal(false); setEditingMilestone(null); }}>×</button>
            </div>
            <div className="modal-body">
              <form onSubmit={async (e) => {
                e.preventDefault();
                ...
                await handleUpdateProjectMilestone({
                  name: formData.get('name'),
                  description: formData.get('description') || null,
                  target_date: formData.get('target_date'),
                  order: parseInt(formData.get('order') as string) || 0
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    name="order"
                    className="form-control"
                    defaultValue={editingMilestone.order || 0}
                    min={0}
                  />
                  <small className="form-text">
                    Order in which milestones appear (lower numbers first)
                ...
                <div className="modal-footer">
                  <button 
                    type="button"
                    className="btn btn-secondary" 
                    onClick={() => { setShowEditProjectMilestoneModal(false); setEditingMilestone(null); }}
                  >
                    Cancel
                  </button>
                  <button type="submit" className="btn btn-primary">
              ...
              <form onSubmit={async (e) => {
                ...
                try {
                  await api.post(`/api/projects/${selectedProject.id}/tasks`, {
                    ...
                    milestone_id: formData.get('milestone_id') ? Number(formData.get('milestone_id')) : null,
                ...
                <div className="form-group">
                  ...
                  >
                    ...
                    {projectMilestones
                      .filter(milestone => !selectedProject || milestone.project_id === selectedProject.id)
                ...
                <div className="form-group">
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      console.log('Available milestones for project', editingTask.project_id, ':', filteredMilestones);
                      console.log('Current milestone_id:', editingTask.milestone_id);
      ...
      {showMilestoneDetailModal && selectedMilestone && (
        <div className="modal-overlay" onClick={() => setShowMilestoneDetailModal(false)}>
          <div className="modal-content" style={{ maxWidth: '900px', maxHeight: '90vh', overflow: 'auto' }} onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="btn-close" onClick={() => setShowMilestoneDetailModal(false)}>×</button>
            </div>
            <div className="modal-body">
              {/* Milestone Info */}
              <div style={{ marginBottom: '20px', padding: '15px', backgroundColor: '#f7fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px', marginBottom: '12px' }}>
                  <div>
                    <div style={{ fontSize: '12px', color: '#718096', marginBottom: '4px' }}>Target Date</div>
                    <div style={{ fontSize: '16px', fontWeight: '600', color: '#2d3748' }}>
                      {new Date(selectedMilestone.target_date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                    </div>
                  </div>
                  <div>
                    ...
                    <div style={{ fontSize: '16px', fontWeight: '600' }}>
                      {selectedMilestone.is_completed ? (
                        <span style={{ color: '#22543d', background: '#c6f6d5', padding: '4px 12px', borderRadius: '12px' }}>✓ Completed</span>
                      ) : new Date(selectedMilestone.target_date) < new Date() ? (
                        <span style={{ color: '#c53030', background: '#fed7d7', padding: '4px 12px', borderRadius: '12px' }}>⚠️ Overdue</span>
                      ) : (
                        <span style={{ color: '#2c5282', background: '#bee3f8', padding: '4px 12px', borderRadius: '12px' }}>📅 In Progress</span>
                      )}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: '12px', color: '#718096', marginBottom: '4px' }}>Progress</div>
                    <div style={{ fontSize: '16px', fontWeight: '600', color: '#2d3748' }}>
                      {(() => {
                        const milestoneTasks = projectTasks.filter(t => t.milestone_id === selectedMilestone.id);
                        const completed = milestoneTasks.filter(t => t.is_completed).length;
                        const total = milestoneTasks.length;
                        const percentage = total > 0 ? Math.round((completed / total) * 100) : 0;
                        return `${percentage}% (${completed}/${total} tasks)`;
                      })()}
                    </div>
                  </div>
                </div>
                ...
                {selectedMilestone.description && (
                  <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px solid #e2e8f0' }}>
                    <div style={{ fontSize: '12px', color: '#718096', marginBottom: '4px' }}>Description</div>
                    <div style={{ fontSize: '14px', color: '#4a5568' }}>{selectedMilestone.description}</div>
                  </div>
                )}
              </div>

              {/* Sub-Projects (Root tasks) */}
              {(() => {
                const rootTasks = projectTasks.filter(t => 
                  t.milestone_id === selectedMilestone.id && !t.parent_task_id
                );
                if (rootTasks.length === 0) return null;
                
                return (
                  <div style={{ marginBottom: '20px' }}>
                    <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#2d3748', marginBottom: '12px', borderBottom: '2px solid #4299e1', paddingBottom: '8px' }}>
                      📁 Sub-Projects ({rootTasks.length})
                    </h3>
                    <div>
                      {rootTasks.map(rootTask => {
                        const allDescendants = projectTasks.filter(t => {
                          let current: ProjectTaskData | undefined = t;
                          while (current && current.parent_task_id) {
                            if (current.parent_task_id === rootTask.id) return true;
                            current = projectTasks.find(p => p.id === current!.parent_task_id);
                          }
                          return false;
                        });
                        const completedDescendants = allDescendants.filter(t => t.is_completed).length;
                        ...
                        return (
                          <div 
                            ...
                            style={{ 
                              padding: '12px', 
                              marginBottom: '8px', 
                              backgroundColor: rootTask.is_completed ? '#f0fff4' : '#fff',
                              border: '1px solid #e2e8f0',
                              borderLeft: `4px solid ${rootTask.is_completed ? '#48bb78' : '#4299e1'}`,
                              borderRadius: '6px'
                            }}
                          >
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <div style={{ flex: 1 }}>
                                <div style={{ fontSize: '15px', fontWeight: '600', color: '#2d3748', textDecoration: rootTask.is_completed ? 'line-through' : 'none' }}>
                                  {rootTask.is_completed && '✓ '}{rootTask.name}
                                </div>
                                {totalDescendants > 0 && (
                                  <div style={{ fontSize: '12px', color: '#718096', marginTop: '4px' }}>
                                    📊 {completedDescendants}/{totalDescendants} subtasks completed
                                  </div>
                                )}
                                {rootTask.due_date && (
                                  <div style={{ fontSize: '12px', color: '#718096', marginTop: '4px' }}>
                                    📅 Due: {new Date(rootTask.due_date).toLocaleDateString()}
                                  </div>
                                )}
                              </div>
                              <button
                                onClick={() => {
                                  handleEditTask(rootTask);
              ...
              {/* All Tasks */}
              {(() => {
                const allTasks = projectTasks.filter(t => t.milestone_id === selectedMilestone.id);
                const groupedByCompletion = {
                  incomplete: allTasks.filter(t => !t.is_completed),
                  completed: allTasks.filter(t => t.is_completed)
                };
                
                return (
                  <div>
                    <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#2d3748', marginBottom: '12px', borderBottom: '2px solid #805ad5', paddingBottom: '8px' }}>
                      ✓ All Tasks ({allTasks.length})
                    </h3>
                    
                    {groupedByCompletion.incomplete.length > 0 && (
                      <div style={{ marginBottom: '20px' }}>
                        <h4 style={{ fontSize: '14px', fontWeight: '600', color: '#4a5568', marginBottom: '8px' }}>
                          📋 Incomplete ({groupedByCompletion.incomplete.length})
                        </h4>
                        {groupedByCompletion.incomplete.map(task => (
                          <div 
                            key={task.id}
                            style={{ 
                              padding: '10px 12px', 
                          ...
                          >
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <div>
                                ...
                                {task.due_date && (
                                  <span style={{ marginLeft: '10px', fontSize: '11px', color: '#718096' }}>
                    ...
                    {groupedByCompletion.completed.length > 0 && (
                      <div>
                        <h4 style={{ fontSize: '14px', fontWeight: '600', color: '#4a5568', marginBottom: '8px' }}>
                          ✓ Completed ({groupedByCompletion.completed.length})
                        </h4>
                        {groupedByCompletion.completed.map(task => (
                          <div 
                            key={task.id}
                            style={{ 
                              padding: '10px 12px', 
